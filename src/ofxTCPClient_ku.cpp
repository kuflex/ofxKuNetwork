...
//--------------------------
ofxTCPClient_ku::ofxTCPClient_ku(){
...
//--------------------------
void ofxTCPClient_ku::setVerbose(bool _verbose){
    ofLogWarning("ofxTCPClient_ku") << "setVerbose(): is deprecated, replaced by ofLogWarning and ofLogError";
}

//--------------------------
bool ofxTCPClient_ku::setup(string ip, int _port, bool blocking){

	if( !TCPClient.Create() ){
		ofLogError("ofxTCPClient_ku") << "setup(): couldn't create client";
		return false;
	}else if( !TCPClient.Connect((char *)ip.c_str(), _port) ){
		ofLogError("ofxTCPClient_ku") << "setup(): couldn't connect to " << ip << " " << _port;
...
//--------------------------
bool ofxTCPClient_ku::setupConnectionIdx(int _index, bool blocking){
	index = _index;
...
//--------------------------
bool ofxTCPClient_ku::close(){
	if( connected ){
		if( !TCPClient.Close() ){
			ofLogError("ofxTCPClient_ku") << "close(): couldn't close client";
			return false;
		}else{
            ofLogVerbose("ofxTCPClient_ku") << "closing client";
			connected = false;
			return true;
		}
	}else{
...
//--------------------------
void ofxTCPClient_ku::setMessageDelimiter(string delim){
	if(delim != ""){
...
//--------------------------
bool ofxTCPClient_ku::send(string message){
	...
	// other applications
	if(!connected){
		ofLogWarning("ofxTCPClient_ku") << "send(): not connected, call setup() first";
		return false;
	}
	...
	if( isClosingCondition(ret, errorCode) ){
		ofLogWarning("ofxTCPClient_ku") << "send(): client disconnected";
		close();
		return false;
	}else if(ret<0){
		ofLogError("ofxTCPClient_ku") << "send(): sending failed";
		return false;
...
//--------------------------
bool ofxTCPClient_ku::sendRawMsg(const char * msg, int size){
	...
	// other applications
	if(!connected){
		ofLogWarning("ofxTCPClient_ku") << "sendRawMsg(): not connected, call setup() first";
		return false;
	}
    ...
    if(ret<0) errorCode = ofxNetworkCheckError();
	if( isClosingCondition(ret, errorCode) ){
		ofLogWarning("ofxTCPClient_ku") << "sendRawMsg(): client disconnected";
		close();
		return false;
	}else if(ret<0){
		ofLogError("ofxTCPClient_ku") << "sendRawMsg(): sending failed";
		return false;
	}else if(ret<size){
		// in case of partial send, store the
		// part that hasn't been sent and send
...
//--------------------------
bool ofxTCPClient_ku::sendRaw(string message){
	if( message.length() == 0) return false;
    int ret = TCPClient.SendAll(message.c_str(), message.length());
    int errorCode = 0;
    if(ret<0) errorCode = ofxNetworkCheckError();
	if( isClosingCondition(ret, errorCode) ){
		ofLogError("ofxTCPClient_ku") << "sendRawBytes(): sending failed";
		close();
		return false;
	}else{
		return ret > 0;
...
//--------------------------
bool ofxTCPClient_ku::sendRawBytes(const char* rawBytes, const int numBytes){
...
//--------------------------
int ofxTCPClient_ku::getNumReceivedBytes(){
	return messageSize;
}
...
//--------------------------
bool ofxTCPClient_ku::isClosingCondition(int messageSize, int errorCode){
...
//--------------------------
string ofxTCPClient_ku::receive(){
	str    = "";
	int length=0;
...
//--------------------------
int ofxTCPClient_ku::receiveRawMsg(char * receiveBuffer, int numBytes){
	int length=-2;
	//only get data from the buffer if we don't have already some complete message
...
//--------------------------
int ofxTCPClient_ku::receiveRawBytes(char * receiveBuffer, int numBytes){
...
//--------------------------
int ofxTCPClient_ku::peekReceiveRawBytes(char * receiveBuffer, int numBytes){
...
//--------------------------
string ofxTCPClient_ku::receiveRaw(){
...
//--------------------------
bool ofxTCPClient_ku::isConnected(){
	if (connected) {
...
//--------------------------
int ofxTCPClient_ku::getPort(){
	return port;
}

//--------------------------
string ofxTCPClient_ku::getIP(){
	return ipAddr;
}
