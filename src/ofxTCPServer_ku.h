...
//------------------------------------------------------------
//ofxTCPServer from ofxNetwork addon modification, just for this:
...
class ofxTCPServer_ku : public ofThread{
	...
	public:
		...
		ofxTCPServer_ku(const ofxTCPServer_ku & mom) = delete;
		ofxTCPServer_ku & operator=(const ofxTCPServer_ku & mom) = delete;
		...
		ofxTCPManager	&TCPServerRef() { return TCPServer; }	//Added by Kuflex

	private:
		ofxTCPClient_ku & getClient(int clientID);
		bool isClientSetup(int clientID);

		void threadedFunction();

		ofxTCPManager			TCPServer;
		std::map<int,std::shared_ptr<ofxTCPClient_ku> >	TCPConnections;
